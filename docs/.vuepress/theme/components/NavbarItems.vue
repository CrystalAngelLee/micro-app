...
import NavbarDropdown from '@vuepress/theme-default/components/NavbarDropdown.vue'
import {
  useRouteLocale,
  useSiteData,
  useSiteLocaleData,
} from '@vuepress/client'
import { isLinkHttp, isString, isPlainObject } from '@vuepress/shared'
...
import { resolveRepoType } from '@vuepress/theme-default/lib/client/utils/index.js'
...
const useNavbarConfig = (): ComputedRef<ResolvedNavbarItem[]> => {
  const themeLocale = useThemeLocaleData()
  /* 👇🏻 microApp custom logic 👇🏻 */
  const router = useRouter()
  const currentPath = router.currentRoute.value.path
  const currentKey = currentPath.replace(/\/(.*?)\/.*/, '$1')
  let navItems = themeLocale.value.navbar || []
  if (navItems.length) {
    const currentItems = navItems.find((item: NavbarItem | NavbarGroup | string) => item.text === currentKey)
    if (isPlainObject(currentItems) && Object.keys(currentItems).length) {
      navItems = currentItems.children || []
    } else {
      navItems = navItems[0].children || []
    }
  }
  /* 👆🏻 microApp custom logic 👆🏻 */
  return computed(() => navItems.map(resolveNavbarItem))
}
